...
export default function Home() {
  const { role, isCaptain, seasonPaused, seasonPauseReason } = useApp()
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background pt-safe">
      ...
      <main className="flex-1 pb-20 pt-4">
        {seasonPaused && (
          <div className="mx-4 mb-4 rounded-lg border border-draw/40 bg-draw/10 p-4 flex items-start gap-3">
            <PauseCircle className="h-5 w-5 text-draw shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-draw">Season Paused</p>
              {seasonPauseReason ? (
                <p className="text-xs text-muted-foreground mt-1">{seasonPauseReason}</p>
              ) : (
                <p className="text-xs text-muted-foreground mt-1">The season is currently on hold. Check back soon for updates.</p>
              )}
            </div>
          </div>
        )}
