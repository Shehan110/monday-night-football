...
function formatOpenTime(rules: LeagueRules): string {
  const day = DAY_NAMES[rules.registrationOpenDay]
  const hour = rules.registrationOpenHour
  const ampm = hour >= 12 ? "PM" : "AM"
  const h = hour % 12 || 12
  return `${day} at ${h}:00 ${ampm}`
}
...
export function Registration() {
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pb-4">
      ...
      {registrationOpen ? (
        <div className="rounded-lg border border-win/30 bg-win/5 p-3 flex items-start gap-2">
          <CheckCircle2 className="h-4 w-4 text-win shrink-0 mt-0.5" />
          <div>
            <p className="text-xs font-bold text-win">Registration is open</p>
            <p className="text-[10px] text-muted-foreground">
              Say &quot;I&apos;m In&quot; to secure your spot for {DAY_NAMES[leagueRules.matchDay]}&apos;s match.
            </p>
          </div>
        </div>
      ) : (
        <div className="rounded-lg border border-draw/30 bg-draw/5 p-3 flex items-start gap-2">
          ...
          <div>
            <p className="text-xs font-bold text-draw">Registration locked</p>
            <p className="text-[10px] text-muted-foreground">
              Opens on {formatOpenTime(leagueRules)}. Players cannot register until then.
            </p>
          </div>
        </div>
      )}
      ...
      {loggedInPlayer && !isAdmin && (
        <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 flex flex-col gap-3">
                    ...
                    : myWaitlistPos > 0
                      ...
                      : loggedInPlayer.registrationStatus === "reserved"
                        ? registrationOpen
                          ...
                          : `Reserved spot - registration opens ${formatOpenTime(leagueRules)}`
                        : registrationOpen
                          ? "Set your availability for this week"
                          : `Registration opens ${formatOpenTime(leagueRules)}`}
                </p>
              </div>
            </div>
          ...
          {!registrationOpen && loggedInPlayer.registrationStatus !== "in" && (
            <div className="rounded-lg border border-draw/30 bg-draw/5 p-3 flex items-start gap-2">
              <Clock className="h-4 w-4 text-draw shrink-0 mt-0.5" />
              <div>
                <p className="text-xs font-bold text-draw">Registration not open yet</p>
                <p className="text-[10px] text-muted-foreground">
                  Spots open on {formatOpenTime(leagueRules)} for next {DAY_NAMES[leagueRules.matchDay]}{"'"}s match.
                  {loggedInPlayer.registrationStatus === "reserved" && " Your reserved spot is held until then."}
                </p>
              </div>
            </div>
          )}
          ...
          {!showDropConfirm ? (
            <div className="flex gap-2">
              <Button
                ...
                disabled={loggedInPlayer.registrationStatus === "in" || (!registrationOpen && loggedInPlayer.registrationStatus !== "in")}
                className={cn(
