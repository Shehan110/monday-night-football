...
import { UserPlus, Trash2, MinusCircle, Pencil, ShieldAlert, Users, AlertTriangle, KeyRound, Check, CalendarClock, Archive, PlayCircle, RotateCcw, Link2, ScrollText, RefreshCw, Lock, Unlock, Settings2, PauseCircle } from "lucide-react"
...
export function AdminPanel() {
  const { players, isAdmin, addPlayer, removePlayer, updatePlayer, changePassword, resetPlayerPassword, seasons, currentSeasonName, endSeason, startNewSeason, setCurrentSeasonName, paymentLink, setPaymentLink, leagueRules, setLeagueRules, startNewMatchWeek, registrationOpen, registrationMode, setRegistrationMode, manualRegistrationOpen, setManualRegistrationOpen, seasonPaused, setSeasonPaused, seasonPauseReason, setSeasonPauseReason } = useApp()
  ...
  // String state for numeric inputs so users can clear fields freely
  const [ruleInputs, setRuleInputs] = useState({
    maxSpots: String(leagueRules.maxSpots),
    winPoints: String(leagueRules.winPoints),
    drawPoints: String(leagueRules.drawPoints),
    lossPoints: String(leagueRules.lossPoints),
    penaltyEarly: String(leagueRules.penaltyEarly),
    penaltyLate: String(leagueRules.penaltyLate),
  })
  const [minusPointsInput, setMinusPointsInput] = useState("-1")
  ...
  const handleMinusPoints = (playerId: string) => {
    ...
    if (!player) return
    const amount = parseInt(minusPointsInput) || -1
    const newMinus = player.minusPoints + amount
        ...
        <CardContent className="p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <PauseCircle className="h-4 w-4 text-draw" />
              <span className="text-xs font-bold text-foreground">Season Pause</span>
            </div>
            <div className={cn(
              ...
              {seasonPaused ? "Paused" : "Active"}
            </div>
          </div>

          <Button
            onClick={() => setSeasonPaused(!seasonPaused)}
            className={cn(
              "h-10 text-xs font-bold w-full",
              seasonPaused
                ? "bg-win text-background hover:bg-win/90"
                : "bg-draw text-background hover:bg-draw/90"
            )}
          >
            {seasonPaused ? (
              <><PlayCircle className="mr-1.5 h-3.5 w-3.5" /> Resume Season</>
            ) : (
              <><PauseCircle className="mr-1.5 h-3.5 w-3.5" /> Pause Season</>
            )}
          </Button>

          {seasonPaused && (
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Reason (shown to players)</label>
              <Textarea
                value={seasonPauseReason}
                onChange={(e) => setSeasonPauseReason(e.target.value)}
                placeholder="e.g. Ramadan break - season resumes after Eid"
                className="bg-secondary text-foreground text-xs min-h-[60px] resize-none"
              />
            </div>
          )}
          ...
          <p className="text-[10px] text-muted-foreground text-center">
            {seasonPaused
              ? "Players will see a banner that the season is paused."
              : "Pause the season for breaks like Ramadan or Muharram."}
          </p>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-foreground">Registration Control</span>
            </div>
            <div className={cn(
              "flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-bold",
              registrationOpen
              ...
              {registrationOpen ? <Unlock className="h-3 w-3" /> : <Lock className="h-3 w-3" />}
          ...
          <div className="flex rounded-lg border border-border overflow-hidden">
            <button
              ...
              className={cn(
                "flex-1 h-10 text-xs font-medium transition-colors flex items-center justify-center gap-1.5",
          ...
          {registrationMode === "auto" ? (
            <p className="text-[10px] text-muted-foreground text-center">
              Opens automatically on {DAY_NAMES[leagueRules.registrationOpenDay]} at {leagueRules.registrationOpenHour % 12 || 12}:00 {leagueRules.registrationOpenHour >= 12 ? "PM" : "AM"}
            </p>
          ) : (
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={manualRegistrationOpen}
                  className={cn(
                ...
                <Button
                  onClick={() => setManualRegistrationOpen(false)}
                  disabled={!manualRegistrationOpen}
                  className={cn(
                    "flex-1 h-10 text-xs font-bold",
                    !manualRegistrationOpen
              ...
              <p className="text-[10px] text-muted-foreground text-center">
                You are manually controlling registration. Auto schedule is paused.
              </p>
            </div>
          )}
        </CardContent>
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input
              ...
              inputMode="numeric"
              pattern="[0-9]*"
              value={ruleInputs.maxSpots}
              onChange={(e) => setRuleInputs((p) => ({ ...p, maxSpots: e.target.value }))}
              onBlur={() => {
                const v = Math.max(2, parseInt(ruleInputs.maxSpots) || 14)
                setLeagueRules({ ...leagueRules, maxSpots: v })
                setRuleInputs((p) => ({ ...p, maxSpots: String(v) }))
              }}
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="flex flex-col gap-0.5">
                ...
                <Input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]*"
                  value={ruleInputs.winPoints}
                  onChange={(e) => setRuleInputs((p) => ({ ...p, winPoints: e.target.value }))}
                  onBlur={() => {
                    const v = Math.max(0, parseInt(ruleInputs.winPoints) || 0)
                    setLeagueRules({ ...leagueRules, winPoints: v })
                    setRuleInputs((p) => ({ ...p, winPoints: String(v) }))
                  }}
                  className="h-9 bg-secondary text-foreground text-sm"
                />
              </div>
              <div className="flex flex-col gap-0.5">
                ...
                <Input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]*"
                  value={ruleInputs.drawPoints}
                  onChange={(e) => setRuleInputs((p) => ({ ...p, drawPoints: e.target.value }))}
                  onBlur={() => {
                    const v = Math.max(0, parseInt(ruleInputs.drawPoints) || 0)
                    setLeagueRules({ ...leagueRules, drawPoints: v })
                    setRuleInputs((p) => ({ ...p, drawPoints: String(v) }))
                  }}
                  className="h-9 bg-secondary text-foreground text-sm"
                />
              </div>
              <div className="flex flex-col gap-0.5">
                ...
                <Input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]*"
                  value={ruleInputs.lossPoints}
                  onChange={(e) => setRuleInputs((p) => ({ ...p, lossPoints: e.target.value }))}
                  onBlur={() => {
                    const v = Math.max(0, parseInt(ruleInputs.lossPoints) || 0)
                    setLeagueRules({ ...leagueRules, lossPoints: v })
                    setRuleInputs((p) => ({ ...p, lossPoints: String(v) }))
                  }}
                  className="h-9 bg-secondary text-foreground text-sm"
                />
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex gap-2 items-center">
                  <Input
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    value={ruleInputs.penaltyEarly}
                    onChange={(e) => setRuleInputs((p) => ({ ...p, penaltyEarly: e.target.value }))}
                    onBlur={() => {
                      const v = Math.max(0, parseInt(ruleInputs.penaltyEarly) || 0)
                      setLeagueRules({ ...leagueRules, penaltyEarly: v })
                      setRuleInputs((p) => ({ ...p, penaltyEarly: String(v) }))
                    }}
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex gap-2 items-center">
                  <Input
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    value={ruleInputs.penaltyLate}
                    onChange={(e) => setRuleInputs((p) => ({ ...p, penaltyLate: e.target.value }))}
                    onBlur={() => {
                      const v = Math.max(0, parseInt(ruleInputs.penaltyLate) || 0)
                      setLeagueRules({ ...leagueRules, penaltyLate: v })
                      setRuleInputs((p) => ({ ...p, penaltyLate: String(v) }))
                    }}
                    className="h-9 w-20 bg-secondary text-foreground text-sm"
                  />
                  <span className="text-[10px] text-muted-foreground">pts</span>
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <Input
                              type="text"
                              inputMode="numeric"
                              value={minusPointsInput}
                              onChange={(e) => setMinusPointsInput(e.target.value)}
                              className="h-9 bg-secondary text-foreground text-sm"
                            />
                          </div>
                          <Button
